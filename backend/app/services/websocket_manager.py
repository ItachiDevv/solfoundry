...
class PubSubAdapter(Protocol):
    """Pub/sub adapter protocol for pluggable backends."""
...
@dataclass
class _RateBucket:
    """Represents a _ rate bucket."""
    timestamps: list = field(default_factory=list)


@dataclass
class _Connection:
    """Represents a _ connection."""
    ws: WebSocket
    user_id: str
...
class WebSocketManager:
    ...
    def _check_rate_limit(self, user_id: str) -> bool:
        """Internal helper: check rate limit."""
        now = time.monotonic()
