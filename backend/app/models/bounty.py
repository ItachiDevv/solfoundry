...
class BountyTier(int, Enum):
    """Bounty difficulty / trust tier."""
    T1 = 1  # open to all, no claiming required
    T2 = 2  # one claim at a time, 7-day deadline, rep >= 10
    T3 = 3  # application required, admin approval, 14-day deadline, rep >= 50
...
class ClaimStatus(str, Enum):
    PENDING = "pending"        # T3 application awaiting approval
    ACTIVE = "active"          # claim is approved / in-progress
    COMPLETED = "completed"    # work accepted
    EXPIRED = "expired"        # deadline passed, auto-released
    RELEASED = "released"      # voluntarily unclaimed
    REJECTED = "rejected"      # T3 application rejected by admin
...
class SubmissionCreate(BaseModel):
    ...
    pr_url: str = Field(..., min_length=1, description="GitHub pull request URL")
    submitted_by: str = Field(..., min_length=1, max_length=100, description="GitHub username")
...
class ClaimRecord(BaseModel):
    """A single claim attempt on a bounty."""
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    bounty_id: str
    contributor_id: str
    status: ClaimStatus = ClaimStatus.ACTIVE
    application_text: Optional[str] = None  # required for T3
...
class BountyBase(BaseModel):
    ...
    deadline: Optional[datetime] = None
    min_reputation: Optional[int] = None  # override tier default
...
class BountyUpdate(BaseModel):
    ...
    @field_validator("required_skills")
    @classmethod
    def normalise_skills(cls, v: list[str] | None) -> list[str] | None:
        if v is None:
            return v
        return [s.strip().lower() for s in v if s.strip()]
...
class BountyListItem(BaseModel):
    """Summary item for list endpoint."""
    id: str
    title: str
    tier: BountyTier
    reward_amount: float
