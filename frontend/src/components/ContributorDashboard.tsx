/**
 * ContributorDashboard - User-specific dashboard showing active bounties,
 * earnings, activity feed, notifications, and account settings.
 *
 * Fetches live data from backend API endpoints with graceful fallback
 * to empty states when the API is unavailable.
 *
 * @module components/ContributorDashboard
 */
'use client';
...
/** Empty defaults used while API data is loading. */
const EMPTY_STATS: DashboardStats = {
  totalEarned: 0, activeBounties: 0, pendingPayouts: 0, reputationRank: 0, totalContributors: 0,
};

/**
 * Fetch dashboard data from real API endpoints.
 *
 * Makes parallel requests to bounties, notifications, and contributors
 * endpoints. Each request fails independently so partial data is
 * still rendered.
 *
 * @param userId - Wallet address or user identifier.
 * @returns Aggregated dashboard data bundle.
 */
async function fetchDashboardData(userId: string | undefined): Promise<DashboardData> {
  const [bountiesResult, notificationsResult, contributorsResult] = await Promise.allSettled([
    apiFetch<{ items: Bounty[] }>('/api/bounties?status=in_progress&limit=10'),
    apiFetch<{ items: Notification[] }>('/api/notifications?limit=20'),
    apiFetch<{ items: unknown[]; total: number }>('/api/contributors?limit=1'),
  ]);

  const bounties = bountiesResult.status === 'fulfilled'
    ? (bountiesResult.value.items || []).map((b: any) => ({
        id: b.id || '', title: b.title || '', reward: b.reward_amount || b.reward || 0,
        deadline: b.deadline || '', status: b.status || 'claimed', progress: b.progress ?? 50,
      }))
    : [];

  const notifications = notificationsResult.status === 'fulfilled'
    ? (notificationsResult.value.items || []).map((n: any) => ({
        id: n.id || '', type: n.type || 'info', title: n.title || '',
        message: n.message || '', timestamp: n.timestamp || n.created_at || '', read: n.read ?? false,
      }))
    : [];

  const totalContributors = contributorsResult.status === 'fulfilled'
    ? contributorsResult.value.total : 0;

  return {
    stats: { ...EMPTY_STATS, activeBounties: bounties.length, totalContributors },
    bounties,
    activities: [],
    notifications,
    earnings: [],
    linkedAccounts: [{ type: 'github', username: userId || '', connected: !!userId }],
  };
}
